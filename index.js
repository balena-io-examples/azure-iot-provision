...
async function handle(method, uuid) {
    try {
        ...
        switch (method) {
            case 'POST':
                console.log(`Creating device: ${uuid} ...`)
                return await handlePost(uuid)
            case 'DELETE':
                console.log(`Deleting device: ${uuid} ...`)
                return await handleDelete(uuid)
            default:
